...
export const paginate = (query: PaginationQueryDto) => {
  ...
  const searchFields = query.searchFields ? query.searchFields : [];
  ...
  if (query.filter) {
    ...
    filter.forEach((f) => {
      const [key, value] = f.split(':') as [string, string];
      paginationParams.where[key] = value;
    });
  }
...
export class Paginate<T> {
  ...
  constructor(private readonly query: PaginationQueryDto) {}
